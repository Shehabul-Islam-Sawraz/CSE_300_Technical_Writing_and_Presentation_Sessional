...
\begin{frame}
\tikzoverlay at (9cm,-4cm) {\includegraphics{lion}};
\titlepage
\end{frame}
...
\begin{frame}{Do you remember the Unit Circle?}
...
%\node(A)[draw]{$sin^2\theta + cos^2\theta = 1$};
{$sin^2\theta + cos^2\theta = 1$}

\node[rectangle callout,color=white, fill=red,  callout relative pointer={(-0.5,0.5)},below right of= A]  {Pythagorean Identity};
...
\node[visible on=<1->]  at  (1,0.5){A B C D E F};
